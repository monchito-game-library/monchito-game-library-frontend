...
import { RETRO_OPTION_PARENT, RetroOptionParent } from '@/retro/retro-select/retro-option-parent.token';
 ...
 * Se combina con la directiva RetroAutocompleteTriggerDirective ([retroAutocompleteTrigger])
...
@Component({
  ...
  template: `
    <ng-template #panel>
      <ul class="retro-autocomplete__panel" [id]="listboxId" role="listbox">
        <ng-content />
      </ul>
    </ng-template>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [{ provide: RETRO_OPTION_PARENT, useExisting: forwardRef(() => RetroAutocompleteComponent) }]
})
export class RetroAutocompleteComponent implements RetroOptionParent, AfterContentInit, OnDestroy {
  ...
  readonly listboxId: string = `retro-autocomplete-listbox-${++_nextAutocompleteId}`;
   ...
   * Llamado por RetroAutocompleteTriggerDirective en ngOnInit.
   *
   ...
   * Llamado por RetroAutocompleteTriggerDirective en ArrowDown/ArrowUp.
   *
   ...
   * Llamado por RetroAutocompleteTriggerDirective en Enter.
   *
