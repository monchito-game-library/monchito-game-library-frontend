...
describe('SupabaseConsoleRepository', () => {
  ...
  describe('updateSaleStatus', () => {
    it('actualiza los campos de venta con id y user_id correctos', async () => {
      ...
      await repo.updateSaleStatus('user-1', 'console-uuid-1', {
        forSale: true,
        salePrice: 299,
        soldAt: null,
        soldPriceFinal: null
      ...
      expect(b.update).toHaveBeenCalledWith(expect.objectContaining({ for_sale: true, sale_price: 299 }));
      expect(b.eq).toHaveBeenCalledWith('id', 'console-uuid-1');
      expect(b.eq).toHaveBeenCalledWith('user_id', 'user-1');
    });
    ...
    it('lanza error si el update de sale status falla', async () => {
      ...
      await expect(
        repo.updateSaleStatus('user-1', 'console-uuid-1', {
          forSale: false,
          salePrice: null,
          soldAt: null,
          soldPriceFinal: null
        })
      ).rejects.toThrow('Failed to update console sale status');
    });
  });
  ...
  describe('createLoan', () => {
    it('inserta el préstamo y devuelve el id creado', async () => {
      ...
      const consoleBuilder = makeBuilder({ error: null });
      mockSupabase.from.mockReturnValueOnce(loanBuilder).mockReturnValueOnce(consoleBuilder);
      ...
      expect(loanBuilder.insert).toHaveBeenCalledWith(
        expect.objectContaining({ item_type: 'console', user_item_id: 'console-uuid-1', loaned_to: 'amigo' })
      );
    });
    ...
    it('lanza error si el insert del préstamo falla', async () => {
      ...
      ).rejects.toThrow('Failed to create console loan');
    });
  });

  describe('returnLoan', () => {
    it('actualiza hardware_loans y limpia los campos de préstamo de la consola', async () => {
      const loanBuilder = makeBuilder({ error: null });
      const consoleBuilder = makeBuilder({ error: null });
      mockSupabase.from.mockReturnValueOnce(loanBuilder).mockReturnValueOnce(consoleBuilder);

      await repo.returnLoan('loan-uuid-1', 'console-uuid-1', 'user-1');

      expect(loanBuilder.update).toHaveBeenCalledWith(expect.objectContaining({ returned_at: expect.any(String) }));
      expect(consoleBuilder.update).toHaveBeenCalledWith(
