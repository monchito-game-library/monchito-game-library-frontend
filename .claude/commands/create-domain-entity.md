...
- Decora la clase con `@Injectable()` (sin `providedIn`). El provider va en `di/use-cases/` (Paso 8), no en el propio decorador.
...
- Al terminar, lista todos los ficheros creados y menciona que falta crear:
  - `src/app/data/repositories/supabase-<entity>.repository.ts` (implementación Supabase)
  - `src/app/data/repositories/supabase-<entity>.repository.spec.ts` (spec con threshold 80% de la app)
