...
describe('ConsolesComponent', () => {
  ...
  describe('onDetail()', () => {
    it('navega a /games/consoles/:id', () => {
      const console1 = makeConsole({ id: 'console-abc' });

      component.onDetail(console1);

      expect(mockRouter.navigate).toHaveBeenCalledWith(['/games/consoles', 'console-abc']);
    });
  });

  describe('onAdd()', () => {
    it('navega a /games/consoles/add', () => {
      component.onAdd();

      expect(mockRouter.navigate).toHaveBeenCalledWith(['/games/consoles/add']);
    });
  });
});
