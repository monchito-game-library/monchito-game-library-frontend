...
class TestDlgComponent {
  data = inject<TestData>(RETRO_DIALOG_DATA);
  ref = inject<RetroDialogRef<TestDlgComponent, string>>(RetroDialogRef);
...
describe('RetroDialogService — integración real', () => {
  beforeEach(() => {
  ...
  it('inyecta RetroDialogRef en el componente abierto', () => {
    const service = TestBed.inject(RetroDialogService);
    ...
    expect(ref.componentInstance!.ref).toBeInstanceOf(RetroDialogRef);
    ref.close();
  });

  it('inyecta RETRO_DIALOG_DATA con los datos pasados en config', () => {
    const service = TestBed.inject(RetroDialogService);
