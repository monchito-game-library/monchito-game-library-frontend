...
const TITLES = ['The Legend of Zelda', 'Final Fantasy VII', 'God of War', 'Metal Gear Solid', 'Resident Evil 4', 'Chrono Trigger', 'Bloodborne', 'Dark Souls', 'Elden Ring', 'Hollow Knight', 'Hades', 'Disco Elysium', 'Red Dead Redemption', 'The Witcher 3', 'Cyberpunk 2077', 'Persona 5', 'Metal Gear Solid 2', 'Silent Hill 2', 'Shadow of the Colossus', 'Bioshock', 'Mass Effect 2', 'Dragon Age Origins', 'Baldurs Gate 3', 'Divinity Original Sin 2', 'Skyrim', 'Oblivion', 'Morrowind', 'Fallout New Vegas', 'Fallout 3', 'Diablo 4', 'Path of Exile', 'Stardew Valley', 'Hollow Knight Silksong', 'Celeste', 'Ori and the Blind Forest', 'Ori Will of the Wisps', 'Inside', 'Limbo', 'Death Stranding', 'Control', 'Alan Wake 2', 'Silent Hill', 'Resident Evil 2', 'Resident Evil Village', 'Resident Evil 7', 'Devil May Cry 5', 'Bayonetta 3', 'Nier Automata', 'Nier Replicant', 'Sekiro'];
const PALETTE = ['#5b3a8c', '#276738', '#a05e0a', '#be1238', '#1e6091', '#7c2d12'];
const fakeCover = (i) => `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 3 4'><rect width='3' height='4' fill='${PALETTE[i % PALETTE.length]}'/></svg>`;

const wishlist = Array.from({ length: 50 }, (_, i) => ({
  id: uuid(i + 7000), user_id: USER_ID, game_catalog_id: uuid(i + 8000),
  title: TITLES[i % TITLES.length] + (i >= TITLES.length ? ` ${Math.floor(i / TITLES.length) + 1}` : ''),
  image_url: fakeCover(i + 100), released_date: '2024-01-15',
  rating: 4, rating_top: 5, genres: ['Action', 'RPG', 'Adventure'].slice(0, 3 + (i % 3)),
  stores: [], tags: [], metacritic_score: null, description: null, source: 'rawg',
  added_by_user_id: USER_ID, times_added_by_users: 0, desired_price: 20 + (i % 50),
  priority: (i % 5) + 1, notes: i % 3 === 0 ? 'Esperando oferta.' : null,
...
for (const vp of VIEWPORTS) {
  ...
  await page.evaluate((s) => localStorage.setItem('sb-egevnihppclxucorhdjt-auth-token', JSON.stringify(s)), FAKE_SESSION);
  ...
  await page.screenshot({ path: `.playwright-mcp/shots/wishlist-split-diagnostic/GRID-${vp.name}-fullpage.png`, fullPage: true });
