-- ============================================================
-- Patch 005 — Fix: image_url debe ser COALESCE(custom, catalog)
-- ============================================================
-- El patch 004 reescribió user_games_full y expuso `gc.image_url`
-- directamente, perdiendo el COALESCE introducido en el fix del bug
-- "Imagen RAWG compartida entre copias" (commit ab8feb0).
--
-- Sin COALESCE, si una copia tiene `ug.custom_image_url` distinto al
-- catálogo (por reposicionamiento o por elegir otro screenshot), la vista
-- ignoraba esa preferencia y todas las copias compartían imagen.
--
-- Esta vista restaura el COALESCE manteniendo el resto del JOIN
-- contra user_works (fase 2). Se rehacen también available_items y
-- sold_items por el DROP CASCADE.
-- ============================================================
